...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    $ci_link = $row['ci_link'] ? unserialize($row['ci_link']) : array();
    $ci_target = $row['ci_target'] ? unserialize($row['ci_target']) : array();
    $ci_img = $row['ci_img'] ? unserialize($row['ci_img']) : array();
    ...
    $ci_subject = $row['ci_subject'] ? unserialize($row['ci_subject']) : array();
